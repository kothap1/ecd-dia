...
def generateTSV(collated, sample_dir, output_file, processing_way):
        ...
        for key, value in sample_grp.items():
            for info in value:
                if info['read_pair'] == 'R1':
                    read1 = f"{sample_dir}/{info['base_dir']}/{info['fastq_name']}"
                elif info['read_pair'] == 'R2':
                    read2 = f"{sample_dir}/{info['base_dir']}/{info['fastq_name']}"
...
if args.samplename and args.manifestdir:
	...
	manifests = []
	for sample in redsheetfile.name_alias:
...
generateTSV(collated, sample_dir, args.output, args.processing)
# }}}--